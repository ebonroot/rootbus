defmodule Utils.Types do
  ...
  def str_to_int!(arg, default \\ 0) do
  ...
  ## Examples
      ...
      {:error, "\"tardis\" is not a number"}
  """
  def str_to_float(arg) when is_number(arg), do: {:ok, arg}
  ...
  def str_to_float!(arg, default \\ 0.0) do
    case str_to_float(arg) do
      {:ok, num} -> num
      {:error, _} -> default
    end
  end
  ...
  With clean=true, go deep, and use clean_atom which will mangle things

    iex> atom_keys(%{"Tardis" => 1, "inSpace" => 2}, clean: true)
    %{tardis: 1, in_space: 2}
    iex> atom_keys([%{ thisCase: 1 }], clean: true)
    [%{ this_case: 1}]
          ...
          {key,
           Enum.map(val, fn elem ->
             cond do
               ...
               is_list(elem) -> elem |> Enum.map(fn e -> atom_keys(e, clean: true) end)
               true -> elem
             end
           end)}
  ...
  def atom_keys(list, opts) when is_list(list) do
    Enum.map(list, fn elem -> atom_keys(elem, opts) end)
  end

  def atom_keys(item, _opts) when is_binary(item) do
    to_atom(item)
  end

  @doc """
  Convert input to atom, not using existing atom validation.  Converts strings,
  atoms, and list of strings or atoms.
  ...
  def to_atom(other) do
    any_to_string(other) |> String.to_atom()
  end

  @doc """
  Convert input to atom, using existing atom validation.  Converts strings,
  atoms, and list of strings or atoms.

    iex> to_atom("long ugly thing")
    iex> to_existing_atom("long ugly thing")
    :"long ugly thing"
    iex> to_existing_atom(:to_atom)
    :to_atom
    iex> to_existing_atom([:to_atom, "to_atom"])
    [:to_atom, :to_atom]
  """
  ...
  def to_existing_atom(list) when is_list(list) do
    Enum.map(list, fn key -> to_existing_atom(key) end)
  end

  def to_existing_atom(key) when is_atom(key), do: key
